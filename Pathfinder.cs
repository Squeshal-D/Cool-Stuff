...
/***********************************
 * A script for Unity C#
 * 
 * Finds a path from a gameobject's position to a point.
 * For use in a top-down 2D setting.
 * Just attach to a gameobject, define its environmental collision box and rigidbody, and it's ready to go.
 * 
 * ********************************/
...
public class Pathfinder : MonoBehaviour
{
    ...
    // testing
    [SerializeField] bool testing = false;                 // if true, lets you click on a spot and draws a path in editor view
    ...
    [SerializeField] float granularity = 1;                // the space between each node (high = blocky + fast, low = accurate + slow)
    ...
    // Converts a node list to a Vector2 list
    ...
    {
        int pathIndex = 0; // Index of position to travel to. Iterates path list.

        while (pathIndex < path.Count)
        {
            rb.velocity = (path[pathIndex] - (Vector2)transform.position).normalized*moveSpeed;

            if ((path[pathIndex] - rb.position).magnitude < 0.1)                                          // character is close to goal
            {
                ...
                {
                    rb.velocity = Vector2.zero;          // Character made it! Stop them there.
